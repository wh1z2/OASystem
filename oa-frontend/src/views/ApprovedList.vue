  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2 text-sm text-gray-500">
        <span>共 {{ approvedList.length }} 项已通过</span>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div v-for="item in approvedList" :key="item.id"
           class="card hover:shadow-md transition-shadow cursor-pointer"
           @click="goToDetail(item.id)">
        <div class="flex items-start gap-4">
          <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 bg-success-50">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-4">
              ...
              <span class="badge badge-success flex-shrink-0">已通过</span>
            </div>
      ...
      <div v-if="!loading && !error && approvedList.length === 0" class="card text-center py-12">
        ...
        <p class="text-gray-500 text-lg">暂无已通过记录</p>
        <p class="text-gray-400 text-sm mt-1">审批通过的申请将显示在这里</p>
      </div>
    </div>
  </div>
</template>
...
const approvedList = computed(() => approvalStore.approvedApprovals)

onMounted(async () => {
  loading.value = true
  error.value = null
  const result = await approvalStore.fetchApprovals({ status: 2 })
  loading.value = false
  if (!result.success) {
